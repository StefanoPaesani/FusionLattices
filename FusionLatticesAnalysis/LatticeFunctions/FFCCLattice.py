...
class FFCCLattice(object):
        ...
        for z_ix in range(self.lattice_z_size):
            for y_ix in range(self.lattice_y_size):
                for x_ix in range(self.lattice_x_size):
                    ...
                    # Layer 1Green
                    qbt_pos = cell_pos + self.xshift + self.yshift + self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][6] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1

                    qbt_pos = cell_pos + self.xshift - 2*self.yshift + self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][7] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1

                    qbt_pos = cell_pos - 2*self.xshift + self.yshift + self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][8] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1
                    ...
                    # Layer 3Red
                    qbt_pos = cell_pos + 3*self.xshift + 3*self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][15] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1

                    qbt_pos = cell_pos + 3*self.xshift - 3*self.yshift + 3*self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][16] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1

                    qbt_pos = cell_pos + 3*self.yshift + 3*self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][17] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1
                    ...
                    # Layer 5Blue
                    qbt_pos = cell_pos + 2*self.xshift - self.yshift + 5*self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][24] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1

                    qbt_pos = cell_pos - self.xshift - self.yshift + 5*self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][25] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1

                    qbt_pos = cell_pos - self.xshift + 2*self.yshift + 5*self.zshift
                    self.primal_qbts_positions[self.current_primal_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 0][26] = self.current_primal_qbt_ix
                    self.current_primal_qbt_ix += 1

                    # Add dual qubits
                    # Layer 0Red
                    qbt_pos = cell_pos + 3*self.xshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][0] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1

                    qbt_pos = cell_pos + 3*self.xshift - 3*self.yshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][1] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1

                    qbt_pos = cell_pos + 3*self.yshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][2] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1
                    ...
                    # Layer 2Blue
                    qbt_pos = cell_pos + 2*self.xshift - self.yshift + 2*self.zshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][9] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1

                    qbt_pos = cell_pos - self.xshift - self.yshift + 2*self.zshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][10] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1

                    qbt_pos = cell_pos - self.xshift + 2*self.yshift + 2*self.zshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][11] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1
                    ...
                    # Layer 4Green
                    qbt_pos = cell_pos + self.xshift + self.yshift + 4*self.zshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][18] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1

                    qbt_pos = cell_pos + self.xshift - 2*self.yshift + 4*self.zshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][19] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1

                    qbt_pos = cell_pos - 2*self.xshift + self.yshift + 4*self.zshift
                    self.dual_qbts_positions[self.current_dual_qbt_ix] = qbt_pos
                    self.cells_qbts_struct[cell_ix, 1][20] = self.current_dual_qbt_ix
                    self.current_dual_qbt_ix += 1
    ...
    def build_lattice_edges(self):
        for cell_ix, all_cell_qbts in enumerate(self.cells_qbts_struct):
            primal_qbts = all_cell_qbts[0]
            dual_qbts = all_cell_qbts[1]

            # Layer 0Red
            self.edged_qbts.append((primal_qbts[0], dual_qbts[0]))
            self.edged_qbts.append((primal_qbts[0], dual_qbts[3]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'z')
            self.edged_qbts.append((primal_qbts[0], self.cells_qbts_struct[neigh_cell][1][21]))

            self.edged_qbts.append((primal_qbts[1], dual_qbts[1]))
            self.edged_qbts.append((primal_qbts[1], dual_qbts[4]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'z')
            self.edged_qbts.append((primal_qbts[1], self.cells_qbts_struct[neigh_cell][1][22]))

            self.edged_qbts.append((primal_qbts[2], dual_qbts[5]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'y')
            self.edged_qbts.append((primal_qbts[2], self.cells_qbts_struct[neigh_cell][1][2]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'z')
            self.edged_qbts.append((primal_qbts[2], self.cells_qbts_struct[neigh_cell][1][23]))

            self.edged_qbts.append((primal_qbts[3], dual_qbts[6]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'x')
            self.edged_qbts.append((primal_qbts[3], self.cells_qbts_struct[neigh_cell][1][0]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'z')
            self.edged_qbts.append((primal_qbts[3], self.cells_qbts_struct[neigh_cell][1][24]))

            self.edged_qbts.append((primal_qbts[4], dual_qbts[7]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'x')
            neigh_cell = self.shifted_cell_ix(neigh_cell, +1, 'y')
            self.edged_qbts.append((primal_qbts[4], self.cells_qbts_struct[neigh_cell][1][1]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'z')
            self.edged_qbts.append((primal_qbts[4], self.cells_qbts_struct[neigh_cell][1][25]))

            self.edged_qbts.append((primal_qbts[5], dual_qbts[8]))
            self.edged_qbts.append((primal_qbts[5], dual_qbts[2]))
            neigh_cell = self.shifted_cell_ix(cell_ix, -1, 'z')
            self.edged_qbts.append((primal_qbts[5], self.cells_qbts_struct[neigh_cell][1][26]))

            # Layer 1Green
            self.edged_qbts.append((primal_qbts[6], dual_qbts[3]))
            self.edged_qbts.append((primal_qbts[6], dual_qbts[8]))
            self.edged_qbts.append((primal_qbts[7], dual_qbts[4]))
            self.edged_qbts.append((primal_qbts[7], dual_qbts[5]))
            self.edged_qbts.append((primal_qbts[8], dual_qbts[6]))
            self.edged_qbts.append((primal_qbts[8], dual_qbts[7]))

            # Layer 2Blue
            self.edged_qbts.append((primal_qbts[9], dual_qbts[9]))
            self.edged_qbts.append((primal_qbts[9], dual_qbts[3]))
            self.edged_qbts.append((primal_qbts[9], dual_qbts[12]))

            self.edged_qbts.append((primal_qbts[10], dual_qbts[9]))
            self.edged_qbts.append((primal_qbts[10], dual_qbts[4]))
            self.edged_qbts.append((primal_qbts[10], dual_qbts[13]))

            self.edged_qbts.append((primal_qbts[11], dual_qbts[10]))
            self.edged_qbts.append((primal_qbts[11], dual_qbts[5]))
            self.edged_qbts.append((primal_qbts[11], dual_qbts[14]))

            self.edged_qbts.append((primal_qbts[12], dual_qbts[10]))
            self.edged_qbts.append((primal_qbts[12], dual_qbts[6]))
            self.edged_qbts.append((primal_qbts[12], dual_qbts[15]))

            self.edged_qbts.append((primal_qbts[13], dual_qbts[11]))
            self.edged_qbts.append((primal_qbts[13], dual_qbts[7]))
            self.edged_qbts.append((primal_qbts[13], dual_qbts[16]))

            self.edged_qbts.append((primal_qbts[14], dual_qbts[11]))
            self.edged_qbts.append((primal_qbts[14], dual_qbts[8]))
            self.edged_qbts.append((primal_qbts[14], dual_qbts[17]))

            # Layer 3Red
            self.edged_qbts.append((primal_qbts[15], dual_qbts[12]))
            neigh_cell = self.shifted_cell_ix(cell_ix, +1, 'x')
            self.edged_qbts.append((primal_qbts[15], self.cells_qbts_struct[neigh_cell][1][15]))

            self.edged_qbts.append((primal_qbts[16], dual_qbts[13]))
            neigh_cell = self.shifted_cell_ix(cell_ix, +1, 'x')
            neigh_cell = self.shifted_cell_ix(neigh_cell, -1, 'y')
            self.edged_qbts.append((primal_qbts[16], self.cells_qbts_struct[neigh_cell][1][16]))

            self.edged_qbts.append((primal_qbts[17], dual_qbts[17]))
            neigh_cell = self.shifted_cell_ix(cell_ix, +1, 'y')
            self.edged_qbts.append((primal_qbts[17], self.cells_qbts_struct[neigh_cell][1][14]))

            # Layer 4Green
            self.edged_qbts.append((primal_qbts[18], dual_qbts[18]))
            self.edged_qbts.append((primal_qbts[18], dual_qbts[12]))
            self.edged_qbts.append((primal_qbts[18], dual_qbts[21]))

            self.edged_qbts.append((primal_qbts[19], dual_qbts[19]))
            self.edged_qbts.append((primal_qbts[19], dual_qbts[13]))
            self.edged_qbts.append((primal_qbts[19], dual_qbts[22]))

            self.edged_qbts.append((primal_qbts[20], dual_qbts[19]))
            self.edged_qbts.append((primal_qbts[20], dual_qbts[14]))
            self.edged_qbts.append((primal_qbts[20], dual_qbts[23]))

            self.edged_qbts.append((primal_qbts[21], dual_qbts[20]))
            self.edged_qbts.append((primal_qbts[21], dual_qbts[15]))
            self.edged_qbts.append((primal_qbts[21], dual_qbts[24]))

            self.edged_qbts.append((primal_qbts[22], dual_qbts[20]))
            self.edged_qbts.append((primal_qbts[22], dual_qbts[16]))
            self.edged_qbts.append((primal_qbts[22], dual_qbts[25]))

            self.edged_qbts.append((primal_qbts[23], dual_qbts[18]))
            self.edged_qbts.append((primal_qbts[23], dual_qbts[17]))
            self.edged_qbts.append((primal_qbts[23], dual_qbts[26]))

            # Layer 5Blue
            self.edged_qbts.append((primal_qbts[24], dual_qbts[21]))
            self.edged_qbts.append((primal_qbts[24], dual_qbts[22]))
            self.edged_qbts.append((primal_qbts[25], dual_qbts[23]))
            self.edged_qbts.append((primal_qbts[25], dual_qbts[24]))
            self.edged_qbts.append((primal_qbts[26], dual_qbts[25]))
            self.edged_qbts.append((primal_qbts[26], dual_qbts[26]))
    ...
    def get_logical_operators(self):
        ...
        while x_ix != 0 or (y_ix != 0) or step_ix == 0:
            if step_ix % 2 == 0:
                for z_ix in range(self.lattice_z_size):
                    ...
                    log_x += list(primal_qubits[[1, 16, 10, 9, 24, 6, 23, 18, 5, 17]])
                y_ix = (y_ix + 1) % self.lattice_x_size
            else:

                for z_ix in range(self.lattice_z_size):
                    cell_ix = self.cell_xyzcoords_to_ix((x_ix, y_ix, z_ix))
                    primal_qubits = self.cells_qbts_struct[cell_ix][0]
                    log_x += list(primal_qubits[[2, 4, 12, 11, 25, 8, 21, 22]])
        ...
        for y_ix in range(self.lattice_y_size):
            for x_ix in range(self.lattice_x_size):
                ...
                primal_qubits = self.cells_qbts_struct[shifted_cell_ix][0]
                log_z += list(primal_qubits[[24, 25, 26]])
    ...
    # Functions that build the matching matrix for syndromes of the lattice
    def get_primal_valencies(self):
        qbts_valencies = np.zeros(self.num_primal_qbts, dtype=np.uint8)
        val_2_cell_primal_qbts = [6, 7, 8, 15, 16, 17, 24, 25, 26]
        for all_cell_qbts in self.cells_qbts_struct:
            primal_qbts = all_cell_qbts[0]
            for cell_qbt_ix, global_qbt_ix in enumerate(primal_qbts):
                if cell_qbt_ix in val_2_cell_primal_qbts:
                    qbts_valencies[global_qbt_ix] = 2
                else:
                    qbts_valencies[global_qbt_ix] = 3
        return qbts_valencies

    # Functions that build the matching matrix for syndromes of the lattice
    def get_primal_valencies_with_branched_for_wiid(self):
        qbts_valencies = np.zeros(self.num_primal_qbts, dtype=np.uint8)
        # val_0_cell_primal_qbts = [6, 7, 8, 15, 16, 17, 24, 25, 26]
        val_2_cell_primal_qbts = [0, 2, 4, 9, 11, 13, 19, 21, 23]
        val_4_cell_primal_qbts = [1, 3, 5, 10, 12, 14, 18, 20, 22]
        for all_cell_qbts in self.cells_qbts_struct:
            primal_qbts = all_cell_qbts[0]
            for cell_qbt_ix, global_qbt_ix in enumerate(primal_qbts):
                if cell_qbt_ix in val_2_cell_primal_qbts:
                    qbts_valencies[global_qbt_ix] = 2
                elif cell_qbt_ix in val_4_cell_primal_qbts:
                    qbts_valencies[global_qbt_ix] = 4
        return qbts_valencies


    # Functions that build the matching matrix for syndromes of the lattice
    def get_primal_valencies_with_branched(self):
        qbts_valencies = np.zeros(self.num_primal_qbts, dtype=np.uint8)
        val_0_cell_primal_qbts = [6, 7, 8, 15, 16, 17, 24, 25, 26]
        for all_cell_qbts in self.cells_qbts_struct:
            primal_qbts = all_cell_qbts[0]
            for cell_qbt_ix, global_qbt_ix in enumerate(primal_qbts):
                if cell_qbt_ix not in val_0_cell_primal_qbts:
                    qbts_valencies[global_qbt_ix] = 1
        return qbts_valencies

    # Functions to deal and move between cells in the lattice
