...
class FusionLattice_Raussendorf_6Rings(object):
    ...
    def __init__(self, lattice_x_size, lattice_y_size, lattice_t_size):
        ...
        self.primal_cells_fus_struct = np.array([[self.empty_ix] * 12 for _ in range(self.num_primal_cells)],
        ...
        for t_ix in range(self.lattice_t_size):
            for y_ix in range(self.lattice_y_size):
                for x_ix in range(self.lattice_x_size):
                    ...
                    ## xy direction
                    this_node_pos = this_cell_pos - self.xshift - self.yshift
                    self.primal_nodes_positions.append(this_node_pos)
                    self.primal_cells_fus_struct[this_cell_ix][6] = self.current_primal_fus_ix
                    neigh_cell = self.shifted_cell_ix(this_cell_ix, -1, 'x')
                    neigh_cell = self.shifted_cell_ix(neigh_cell, -1, 'y')
                    self.primal_cells_fus_struct[neigh_cell][7] = self.current_primal_fus_ix
                    self.current_primal_fus_ix += 1

                    ## xt direction
                    this_node_pos = this_cell_pos - self.xshift - self.tshift
                    self.primal_nodes_positions.append(this_node_pos)
                    self.primal_cells_fus_struct[this_cell_ix][8] = self.current_primal_fus_ix
                    neigh_cell = self.shifted_cell_ix(this_cell_ix, -1, 'x')
                    neigh_cell = self.shifted_cell_ix(neigh_cell, -1, 't')
                    self.primal_cells_fus_struct[neigh_cell][9] = self.current_primal_fus_ix
                    self.current_primal_fus_ix += 1

                    ## yt direction
                    this_node_pos = this_cell_pos - self.yshift - self.tshift
                    self.primal_nodes_positions.append(this_node_pos)
                    self.primal_cells_fus_struct[this_cell_ix][10] = self.current_primal_fus_ix
                    neigh_cell = self.shifted_cell_ix(this_cell_ix, -1, 'y')
                    neigh_cell = self.shifted_cell_ix(neigh_cell, -1, 't')
                    self.primal_cells_fus_struct[neigh_cell][11] = self.current_primal_fus_ix
                    self.current_primal_fus_ix += 1
