...
try:
    from LatticeFunctions.FFCCLattice_FusionArbChains import FFCCLattice_FusionsChains
...
def decoder_successprob_error_vs_loss_list_parallelized(error_vs_loss_list, L, chains_len, log_op_axis='z',
                                                        error_type='iid',
    ...
    Lattice = FFCCLattice_FusionsChains(L, L, L, chains_lenght=chains_len)

    #### Get matching matrix and define Pymatching matching object
    H_matrix = Lattice.get_matching_matrix()
    num_primal_qbts_full = H_matrix.shape[1]
    used_qbts, qbt_syndr_mat = np.where(H_matrix.T)
    num_primal_qbts =  int(len(qbt_syndr_mat) / 2)
    qbt_syndr_mat = qbt_syndr_mat.reshape((num_primal_qbts, 2)).astype(dtype=np.int32)
    used_qbts = used_qbts[::2].astype(dtype=np.int32)
    H_matrix = H_matrix[:, used_qbts]


    log_op_qbts0 = Lattice.log_ops_qbts[log_op_ix]
    log_op_array = np.array([1 if x in log_op_qbts0 else 0 for x in range(num_primal_qbts_full)],
                            dtype=np.uint8)
    log_op_array = log_op_array[used_qbts]

    H_withlogop = np.vstack([H_matrix, log_op_array])

    pool = multiprocessing.Pool()
    success_probs = pool.starmap(decoder_successprob_error_vs_loss_single_vals,
                                 zip(error_vs_loss_list, repeat(num_primal_qbts), repeat(H_withlogop),
...
if __name__ == '__main__':
    ...
    for L_ix, L in enumerate(L_list):
        print('   Doing L=', L)
        this_data = \
            decoder_successprob_error_vs_loss_list_parallelized(err_vs_eras_vals, L, chains_len,
    ...
    plt.legend()
    plt.savefig(
        os.path.join(saving_folder, 'FFCCLatticeFusion' + str(chains_len) + 'Chains_lossonly.png'))
    ...
    for L_ix, L in enumerate(L_list):
        print('   Doing L=', L)
        this_data = \
            decoder_successprob_error_vs_loss_list_parallelized(err_vs_eras_vals, L, chains_len, log_op_axis=log_op_dir,
                                                                num_loss_trials=num_trials,
                                                                num_ec_runs_per_loss_trial=1)
    ...
    plt.ylabel("Logical error rate")
    plt.legend()
    plt.savefig(os.path.join(saving_folder, 'FFCCLatticeFusion'+str(chains_len)+'Chains_errorsonly.png'))
    ...
    #     for L_ix, L in enumerate(L_list):
    #         print('   Doing L=', L)
    #         this_data = decoder_successprob_error_vs_loss_list_parallelized(err_vs_eras_vals, L, chains_len, log_op_axis=log_op_dir,
    ...
    #                              'PlotDuring2DScan_FFCCLatticeFusion'+str(chains_len)+'Chains_scan'+str(scan_ix)+'of'+str(num_scans)+ '_logopaxis_' + log_op_dir +'.png'))
    #
    # file_name = 'error_vs_erasure_FFCCLatticeFusion'+str(chains_len)+'Chains_Lmax' + str(max(L_list)) + '_logopaxis_' + log_op_dir +'_numlosstrials' + str(
