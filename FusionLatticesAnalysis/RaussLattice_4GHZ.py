...
try:
    from LatticeFunctions.FusionRaussendorf_4GHZ import FusionLattice_Raussendorf_4GHZ
...
def decoder_successprob_error_vs_loss_list_parallelized(error_vs_loss_list, L, error_type='iid',
                                                        num_loss_trials=1000, num_ec_runs_per_loss_trial=1):


    Lattice = FusionLattice_Raussendorf_4GHZ(L, L, L)
    num_fusions = Lattice.num_primal_fusions

    log_op = Lattice.log_ops_fus[0]
    log_op_array = np.array([1 if x in log_op else 0 for x in range(num_fusions)],
                            dtype=np.uint8)

    #### Get matching matrix and define Pymatching matching object
    H_matrix = Lattice.get_matching_matrix()
    qbt_syndr_mat = np.where(H_matrix.T)[1].reshape((num_fusions, 2)).astype(dtype=np.int32)
    H_withlogop = np.vstack([H_matrix, log_op_array])

    pool = multiprocessing.Pool()
    success_probs = pool.starmap(decoder_successprob_error_vs_loss_single_vals,
                                 zip(error_vs_loss_list, repeat(num_fusions), repeat(H_withlogop),
...
if __name__ == '__main__':
    ...
    for L_ix, L in enumerate(L_list):
        ...
        this_data = \
            decoder_successprob_error_vs_loss_list_parallelized(4*err_vs_eras_vals, L, num_loss_trials=num_trials,
                                                                num_ec_runs_per_loss_trial=1)
    ...
    plt.legend()
    plt.savefig(os.path.join(saving_folder, 'RaussLattice_4GHZ_lossonly.png'))
    ...
    for L_ix, L in enumerate(L_list):
        print('   Doing L=', L)
        this_data = \
            decoder_successprob_error_vs_loss_list_parallelized(4*err_vs_eras_vals, L, #Takes into account that there are 4-degenerate edges in the cubic syndrome graph
    ...
    plt.ylabel("Logical error rate")
    plt.legend()
    plt.savefig(os.path.join(saving_folder, 'RaussLattice_4GHZ_errorsonly_.png'))
    ...
    #     for L_ix, L in enumerate(L_list):
    #         print('   Doing L=', L)
    #         this_data = decoder_successprob_error_vs_loss_list_parallelized(4*err_vs_eras_vals, L, #Takes into account that there are 4-degenerate edges in the cubic syndrome graph
    ...
    #                              'PlotDuring2DScan_RaussLattice4GHZ_scan'+str(scan_ix)+'of'+str(num_scans)+'.png'))
    #
    # file_name = 'error_vs_erasure_Raussendorf4GHZ_Lmax' + str(max(L_list)) + '_numlosstrials' + str(
