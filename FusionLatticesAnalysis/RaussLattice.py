...
from LatticeFunctions.RaussendorfLattice import RaussendorfLattice
...
def decoder_successprob_error_vs_loss_single_vals(noise_vals, num_primal_qbts, H_withlogop, log_op_array, qbt_syndr_mat,
    ...
    if loss_p > 0:
        ...
        if error_type == 'iid':
            matching = Matching(H_withlogop[:-1], error_probabilities=error_p)
        else:
            raise ValueError('Only accepted error types are: ["iid"]')
        for _ in range(num_loss_trials * num_ec_runs_per_loss_trial):
            noise, syndrome = matching.add_noise()
            correction = matching.decode(syndrome)
            num_errors += np.dot(log_op_array, (correction + noise)) % 2
    return (num_errors / (num_loss_trials * num_ec_runs_per_loss_trial))


def decoder_successprob_error_vs_loss_list_parallelized(error_vs_loss_list, L, error_type='iid', num_loss_trials=1000,
                                                        num_ec_runs_per_loss_trial=1, boundary_is_primal=(
                (True, False), (True, False), (True, False)), boundary_is_periodic=True):
    Rauss_Lattice = RaussendorfLattice(L, L, L, boundary_is_primal, boundary_is_periodic)
    log_op_qbts0 = Rauss_Lattice.log_ops_qbts[0]
    log_op_array = np.array([1 if x in log_op_qbts0 else 0 for x in range(Rauss_Lattice.num_primal_qbts)],
                            dtype=np.uint8)

    #### Get matching matrix and define Pymatching matching object
    H_matrix = Rauss_Lattice.get_matching_matrix(latt_type='primal')
    qbt_syndr_mat = np.where(H_matrix.T)[1].reshape((Rauss_Lattice.num_primal_qbts, 2)).astype(dtype=np.int32)
    H_withlogop = np.vstack([H_matrix, log_op_array])

    pool = multiprocessing.Pool()
    success_probs = pool.starmap(decoder_successprob_error_vs_loss_single_vals, \
                                 zip(error_vs_loss_list, repeat(Rauss_Lattice.num_primal_qbts), repeat(H_withlogop),
                                     repeat(log_op_array), repeat(qbt_syndr_mat), \
                                     repeat(error_type), repeat(num_loss_trials), repeat(num_ec_runs_per_loss_trial)))
...
if __name__ == '__main__':
    ...
    plt.legend()
    plt.savefig(os.path.join(saving_folder, 'RaussLattice_lossonly.png'))
    ...
    plt.ylabel("Logical error rate")
    plt.legend()
    plt.savefig(os.path.join(saving_folder, 'RaussLattice_errorsonly.png'))
    ...
    #     for L_ix, L in enumerate(L_list):
    #         print('   Doing L=', L)
    #         this_data = decoder_successprob_error_vs_loss_list_parallelized(err_vs_eras_vals, L,
    ...
    #                              'PlotDuring2DScan_RaussLattice_scan'+str(scan_ix)+'of'+str(num_scans)+'.png'))
    #
    # file_name = 'error_vs_erasure_Raussendorf_Lmax' + str(max(L_list)) + '_numlosstrials' + str(
