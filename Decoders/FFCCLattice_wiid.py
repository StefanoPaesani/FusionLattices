...
from LatticeFunctions.FFCCLattice import FFCCLattice
...
def decoder_single_run_lossandEC(num_primal_qbts, H_withlogop, qbt_syndr_mat, vals_qubits, err_p, loss_p,
                                 error_type='weight-iid',
    ...
    if error_type == 'iid':
        ...
        lost_qubits = np.random.binomial(1, loss_p).astype(np.uint8)
    else:
    ...
    if num_lost_qbts:
        ...
        vals_qubits_ordered = vals_qubits[lostfirst_qbts_order]
        ...
        new_logop = Hwithlogop_ordered_rowechelon[-1]
        vals_qubits_ordered = vals_qubits
    ...
    if err_p > 1.e-10:
        ...
        new_vals_qubits = vals_qubits_ordered[num_lost_qbts:]
        ...
        error_probs = get_multiedge_errorprob(error_type=error_type, p=err_p, valencies_list=new_vals_qubits,
                                              inv_mat=inverse_ixs, num_good_qbts=len(new_ixs))
...
def decoder_successprob_error_vs_loss_single_vals(noise_vals, num_primal_qbts, H_withlogop, vals_qubits, qbt_syndr_mat,
    ...
    if loss_p > 0:
        if error_type == 'iid':
            for loss_trial in range(num_loss_trials):
                num_errors += decoder_single_run_lossandEC(num_primal_qbts, H_withlogop, qbt_syndr_mat, vals_qubits,
                                                           error_p, loss_p,
        ...
        if error_type == 'iid':
            ...
            error_probs = get_multiedge_errorprob(error_type=error_type, p=error_p, valencies_list=vals_qubits,
                                                  inv_mat=inverse_ixs, num_good_qbts=len(new_ixs))
            NewH_weights = get_Hmat_weights(error_type=error_type, multiedge_error_probs=error_probs)
...
def decoder_successprob_error_vs_loss_list_parallelized(error_vs_loss_list, L, log_op_axis='z', error_type='weight-iid',
    ...
    ### Get qubits valencies:
    vals_qubits = Lattice.get_primal_valencies()
    ...
    success_probs = pool.starmap(decoder_successprob_error_vs_loss_single_vals,
                                 zip(error_vs_loss_list, repeat(Lattice.num_primal_qbts), repeat(H_withlogop),
                                     repeat(vals_qubits), repeat(qbt_syndr_mat), repeat(error_type),
...
if __name__ == '__main__':
    ...
    plt.legend()
    plt.savefig(os.path.join(saving_folder, 'FFCCLattice_wiid_lossonly.png'))
    ...
    plt.ylabel("Logical error rate")
    plt.legend()
    plt.savefig(os.path.join(saving_folder, 'FFCCLattice_wiid_errorsonly.png'))
    ...
    #                                                                         num_ec_runs_per_loss_trial=num_ec_runs_per_loss_trial)
    #         data_this_scan.append(np.array(this_data))
    ...
    #                              'PlotDuring2DScan_FFCCLattice_wiid_scan'+str(scan_ix)+'of'+str(num_scans)+ '_logopaxis_' + log_op_dir +'.png'))
    #
    #     file_name = 'savingscan_'+str(scan_ix)+'of'+str(num_scans)+'_errvseras_FFCCLattice_wiid'+ '_logopaxis_' + log_op_dir+'_Lmax' + str(max(L_list)) + '_numlosstrials' + str(
    #         num_loss_trials) + '_numecrunsperlosstrial' + str(num_ec_runs_per_loss_trial) + '_numsteps' + \
    #                 str(num_steps)+ '.pickle'
    #     data_to_save = [L_list, err_vs_eras_vals, data_this_scan]
    #     with open(os.path.join(saving_folder, file_name), 'wb') as handle:
    #         pickle.dump(data_to_save, handle)
    #
    # file_name = 'error_vs_erasure_FFCCLattice_wiid_Lmax' + str(max(L_list))+ '_logopaxis_' + log_op_dir + '_numlosstrials' + str(
    #     num_loss_trials) + '_numecrunsperlosstrial' + str(num_ec_runs_per_loss_trial) + '_numsteps' + \
